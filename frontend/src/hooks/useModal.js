...
const useModal = () => {
  ...
  // This turned out a bit wonky, since when the similar photos element is hidden, the height of
  // the modal doesn't require scrolling.  So you get smooth scrolling when you show the similar photos,
  // but an unavoidable snap to top when you close it.  The solution might just be to eliminate the
  // show/hide functionality altogether.
  useEffect(() => {
    if (showSimilar) {
      scrollTo('similar');
    }
